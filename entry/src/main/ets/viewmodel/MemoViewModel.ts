...
export class MemoViewModel {
  ...
  /**
   * Adds a new memo to the database and refreshes the list.
   * @param title The title of the new memo.
   * @param content The content of the new memo.
   */
  async addMemo(title: string, content: string) {
    if (!this.context) {
      console.error('MemoViewModel', 'Context not initialized before adding memo.');
      return;
    }
    this.isLoading = true;
    console.info('MemoViewModel', 'Adding new memo...');
    try {
      const newMemo = new Memo(title, content); // Create Memo object (timestamps are auto-set)
      const rowId = await memoDao.addMemo(this.context, newMemo);
      if (rowId > 0) {
        console.info('MemoViewModel', 'Memo added successfully. Refreshing list...');
        await this.searchMemos(this.searchKeyword); // Refresh the list based on current search
      } else {
        console.error('MemoViewModel', 'Failed to add memo (DAO returned invalid rowId).');
        // Optionally show an error message
      }
    } catch (error) {
      console.error('MemoViewModel', `Error adding memo: ${JSON.stringify(error)}`);
    } finally {
      this.isLoading = false;
    }
  }
  ...
  /**
   * Updates an existing memo in the database and refreshes the list.
   * @param memo The Memo object with updated details (must include id).
   */
  async updateMemo(memo: Memo) {
    if (!this.context) {
      console.error('MemoViewModel', 'Context not initialized before updating memo.');
      return;
    }
    if (memo.id === undefined) {
      console.error('MemoViewModel', 'Cannot update memo without an ID.');
      return;
    }
    this.isLoading = true;
    // Update the modifiedDate before saving
    memo.modifiedDate = Date.now();
    console.info('MemoViewModel', `Updating memo with ID: ${memo.id}...`);
    try {
      const affectedRows = await memoDao.updateMemo(this.context, memo);
      if (affectedRows > 0) {
        console.info('MemoViewModel', 'Memo updated successfully. Refreshing list...');
      ...
      console.error('MemoViewModel', `Error updating memo: ${JSON.stringify(error)}`);
    } finally {
      this.isLoading = false;
    }
  }
